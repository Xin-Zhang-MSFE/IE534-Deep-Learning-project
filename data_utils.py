# -*- coding: utf-8 -*-
"""
Created on Tue May 11 01:16:56 2021
...
import numpy as np
'''
TRAIN_PATH = "yahoo train.csv"
TEST_PATH = "yahoo test.csv"
'''
TRAIN_PATH = "Yelp Review Polarity train.csv"
TEST_PATH = "Yelp Review Polarity test.csv"
...
def build_word_dict():
    if not os.path.exists("word_dict.pickle"):
        train_df = pd.read_csv(TRAIN_PATH, names=["class", "title",'title2', "content"],nrows=10000)
        del train_df['title2']
...
def build_char_dataset(step, model, document_max_len):
    ...
    if step == "train":
        df = pd.read_csv(TRAIN_PATH, names=["class", "content"],nrows=10000)#, "title",'title2',
        #del df['title2']
    else:
        df = pd.read_csv(TEST_PATH, names=["class", "content"],nrows=1000)
        #del df['title2']
    ...
    x = list(map(lambda content: list(map(lambda d: char_dict.get(d, char_dict["<unk>"]), str(content).lower())), df["content"]))
    x = list(map(lambda d: d[:document_max_len], x))
